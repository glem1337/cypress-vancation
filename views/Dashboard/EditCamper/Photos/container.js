...
class ListingPhotos extends React.Component {
  ...
  static getInitialProps = async (ctx) => {
    ...
    ctx.store.dispatch(fetchCamper(camperId, CAMPER_INCLUSION.CAMPER_PHOTOS));
  ...
  uploadPhotos = () => {
    ...
    if (
      ...
      || camperPhotos.length >= FORM_VALIDATION.MAX_PHOTOS_COUNT
    ) {
      return false;
    }
  ...
  checkUploadButtonPosition = () => {
    const sortSection = document.getElementById(
      'camper-photo__sortable-section',
    );
