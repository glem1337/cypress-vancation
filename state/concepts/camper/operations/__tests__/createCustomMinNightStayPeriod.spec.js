...
import mockedResponse from 'state/concepts/camper/__mocks__/createCustomMinNightStayPeriodResponse';
...
import createCustomMinNightStayPeriod from '../createCustomMinNightStayPeriod';

jest.mock('utils/showErrorNotifications', () => jest.fn());

const mockedCurrentDate = moment();
jest.mock('state/concepts/calendar/selectors', () => ({
  customMinNightStayIdsSelector: jest.fn(() => []),
...
describe('createCustomMinNightStayPeriod', () => {
  let dispatch;
  ...
  const beforeFunction = httpClient => () => {
    dispatch = jest.fn();
    createCustomMinNightStayPeriod.process(
      { httpClient, action, getState: jest.fn() }, dispatch, jest.fn(),
    );
  };

  it('has valid attributes', () => {
    expect(createCustomMinNightStayPeriod).toMatchSnapshot();
  });
  ...
  describe('success', () => {
    ...
    it('success', () => {
      ...
      expect(dispatch).toHaveBeenNthCalledWith(
        3,
        setCustomMinNightStayIds([mockedResponse.data.data.id]),
      );
